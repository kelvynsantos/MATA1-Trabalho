...
// Função que define os arcos e seus pesos
int grafo(int g[V][V], int n)
{
    ...
    for (i = 1 ; i <= n ; i++)
    {
        for (j = 1 ; j <= n ; j++)
        {
            ...
            printf("\nOs vertices %d e %d formam um arco? (S/N):",i , j);
            scanf(" %c", &resposta);
...
// Função que implementa o algoritmo de Dijkstra em um digrafo (grafo orientado)
